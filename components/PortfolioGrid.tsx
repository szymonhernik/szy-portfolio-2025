...
export default function PortfolioGrid() {
  return (
    <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
      {items.map((item) => (
        <Link key={`portfolio-item-${item.id}`} href="#" className="group block">
          <div className="flex h-full flex-col justify-end">
            <div className="relative mb-4 w-full" style={{ aspectRatio: item.aspectRatio }}>
              <Image src={item.imageUrl || "/placeholder.svg"} alt={item.title} fill className="" />
            </div>
            <div>
